...
public class ThrottleSettingsFragment extends Fragment implements OnSeekBarChangeListener, OnLongClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (app.settings != null)
		{
			...
			txtChannelName.setText("THROTTLE");
			...
			chkReverse.setChecked(app.settings.RC3_REV == 1 ? true : false);
			
			txtNegDR = (TextView)v.findViewById(R.id.txtNegDR);
			txtNegDR.setText(strNegDR + app.settings.THROTTLE_NEG_DR);
			...
			seekBarNegDR.setProgress(app.settings.THROTTLE_NEG_DR);
			
			txtPosDR = (TextView)v.findViewById(R.id.txtPosDR);
			txtPosDR.setText(strPosDR + app.settings.THROTTLE_POS_DR);
			...
			seekBarPosDR.setProgress(app.settings.THROTTLE_POS_DR);
			
			txtNegEXP = (TextView)v.findViewById(R.id.txtNegEXP);
			txtNegEXP.setText(strNegEXP + app.settings.THROTTLE_NEG_EXP);
			...
			seekBarNegEXP.setProgress(app.settings.THROTTLE_NEG_EXP + 100);
			
			txtPosEXP = (TextView)v.findViewById(R.id.txtPosEXP);
			txtPosEXP.setText(strPosEXP + app.settings.THROTTLE_POS_EXP);
			...
			seekBarPosEXP.setProgress(app.settings.THROTTLE_POS_EXP + 100);
			
			RelativeLayout rl = (RelativeLayout)v.findViewById(R.id.rlGraph);
			surfaceViewGraph = new GraphWidget(getActivity(), app.settings.THROTTLE_NEG_DR, app.settings.THROTTLE_POS_DR, app.settings.THROTTLE_NEG_EXP, app.settings.THROTTLE_POS_EXP);
			...
			vsbChannel.setMax(app.settings.RC3_MAX - app.settings.RC3_MIN);
	...
	private OnCheckedChangeListener onCheckReverseChanged = new OnCheckedChangeListener() {		
		...
		public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
			if (app.settings != null)
			{
				...
				app.settings.RC3_REV = (isChecked == true ? 1 : 0);

				SharedPreferences prefs = getActivity().getSharedPreferences(getActivity().getPackageName(), Context.MODE_PRIVATE);
	    	    SharedPreferences.Editor editor = prefs.edit();
	    	    editor.putInt("RC3_REV", app.settings.RC3_REV);
	    	    editor.commit();
			}
		}
	};
	...
	public void onProgressChanged(SeekBar seekBar, int progress,
		...
		if (app.settings != null)
		{
			...
			case R.id.seekBarNegDR:
				SettingsActivity.bEdited = true;
				app.settings.THROTTLE_NEG_DR = progress;
				txtNegDR.setText(strNegDR + app.settings.THROTTLE_NEG_DR);
				break;
			case R.id.seekBarPosDR:
				SettingsActivity.bEdited = true;
				app.settings.THROTTLE_POS_DR = progress;
				txtPosDR.setText(strPosDR + app.settings.THROTTLE_POS_DR);
				break;
			case R.id.seekBarNegEXP:
				SettingsActivity.bEdited = true;
				app.settings.THROTTLE_NEG_EXP = progress - 100;
				txtNegEXP.setText(strNegEXP + app.settings.THROTTLE_NEG_EXP);
				break;
			case R.id.seekBarPosEXP:
				SettingsActivity.bEdited = true;
				app.settings.THROTTLE_POS_EXP = progress - 100;
				txtPosEXP.setText(strPosEXP + app.settings.THROTTLE_POS_EXP);
				break;
			case R.id.vsbChannel:
				float valNorm = normalize(progress, app.settings.RC3_MAX - app.settings.RC3_MIN);
				if (valNorm != 0)
					surfaceViewGraph.updateCursor(valNorm);
				int x = progress - ((app.settings.RC3_MAX - app.settings.RC3_MIN)/2);
				int y = calculateY(valNorm);
				txtChannelVal.setText("X: " + x + "\nY: " + y);
				break;
			}		
			if (surfaceViewGraph != null)
				surfaceViewGraph.update(app.settings.THROTTLE_NEG_DR, app.settings.THROTTLE_POS_DR, app.settings.THROTTLE_NEG_EXP, app.settings.THROTTLE_POS_EXP);
		}
	}
	...
	{
		float negativeDR = app.settings.THROTTLE_NEG_DR / 100.0f;
		float positiveDR = app.settings.THROTTLE_POS_DR / 100.0f;
		float negativeEXP = app.settings.THROTTLE_NEG_EXP;
		float fnegativeEXP = negativeEXP / 100.0f;
		float positiveEXP = app.settings.THROTTLE_POS_EXP;
		...
		return (int) ((app.settings.RC3_MIN+((app.settings.RC3_MAX-app.settings.RC3_MIN)/2)) + ((app.settings.RC3_MAX-app.settings.RC3_MIN)/2) * fval);
	}
	...
	public void onStopTrackingTouch(SeekBar seekBar) {
		if (app.settings != null)
		{
			...
			{
			case R.id.seekBarNegDR:	  	    	
				editor.putInt("THROTTLE_NEG_DR", app.settings.THROTTLE_NEG_DR);
				editor.commit();
				break;
			case R.id.seekBarPosDR:	  	    	
				editor.putInt("THROTTLE_POS_DR", app.settings.THROTTLE_POS_DR);
				editor.commit();
				break;
			case R.id.seekBarNegEXP:	  	    	
				editor.putInt("THROTTLE_NEG_EXP", app.settings.THROTTLE_NEG_EXP);
	    	    editor.commit();
				break;
			case R.id.seekBarPosEXP:	  	    	
				editor.putInt("THROTTLE_POS_EXP", app.settings.THROTTLE_POS_EXP);
				editor.commit();
				break;
			case R.id.vsbChannel:
				vsbChannel.setProgress(vsbChannel.getMax() / 2);
